...
def command_switch_relative(direction: str) -> None:
    sessions = list_sessions()
    current_id = current_session_id()
    indices = {s["id"]: i for i, s in enumerate(sessions)}
    if current_id not in indices:
        return
    i = indices[current_id]
    if direction == "left" and i > 0:
        target = sessions[i - 1]
    elif direction == "right" and i < len(sessions) - 1:
        target = sessions[i + 1]
    else:
        return
...
def main(argv: List[str]) -> None:
    ...
    elif command == "switch-relative" and len(argv) >= 3:
        command_switch_relative(argv[2])
